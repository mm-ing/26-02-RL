"""
UI layer for the Reacher RL Workbench.
...
class ConfigPanel(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        _row(fe, "Ctrl Weight", self._reward_ctrl_w,   0, 0, 7)
        _row(fe, "Dist Weight", self._reward_dist_w,   0, 2, 7)
        _row(fe, "Render ms",   self._render_interval, 1, 0, 7)
        ...
        self._max_steps  = tk.IntVar(value=50)
        ...
        ttk.Combobox(ft, textvariable=self._tune_algo_var,
                     ...
                     width=8).grid(row=1, column=1, sticky="ew",
                                   padx=(0, 6), pady=2)
        ttk.Label(ft, text="Parameter").grid(row=1, column=2, sticky="w",
                                             padx=(4, 2))
        ...
        ttk.Label(ft, text="Values (;)").grid(row=2, column=0, sticky="w",
                                              padx=(4, 2))
        ...
        ttk.Button(bf, text="Add to List",
    ...
    def _build_ppo_tab(self) -> ttk.Frame:
        ...
        ttk.Combobox(tab, textvariable=self._ppo_act,
                     values=["relu", "tanh", "elu"], state="readonly",
                     width=7).grid(row=4, column=3, sticky="ew",
                                   padx=(0, 4), pady=2)
        return tab
    ...
    def _build_sac_tab(self) -> ttk.Frame:
        ...
        self._sac_starts  = tk.IntVar(value=1_000)
    ...
    def _build_td3_tab(self) -> ttk.Frame:
        ...
        self._td3_starts   = tk.IntVar(value=1_000)
        ...
        ttk.Combobox(tab, textvariable=self._td3_act,
                     values=["relu", "tanh", "elu"], state="readonly",
                     width=7).grid(row=5, column=3, sticky="ew",
                                   padx=(0, 4), pady=2)
        return tab
    ...
    def _tune_add(self) -> None:
        ...
        self._tune_items.append((algo, param, values, iid))

    def _tune_remove(self) -> None:
        sel = self._tune_tree.selection()
        if sel:
            self._tune_tree.delete(sel[0])
            self._tune_items = [i for i in self._tune_items if i[3] != sel[0]]
...
class VisualizationPanel(ttk.Frame):
    ...
    def update_frame(self, frame) -> None:
        ...
        self._canvas.delete("all")
        self._canvas.create_image(self._cw // 2, self._ch // 2,
...
class TrainingStatusWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        self._tv = ttk.Treeview(
            frm, columns=self.COLS, show="headings", selectmode="browse")
        headings = {
...
class WorkbenchApp(tk.Tk):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("RL Workbench – Reacher-v5")
    ...
    def _build_layout(self) -> None:
        ...
        # top paned: left 1/3 config | right 2/3 viz
        top_pw = ttk.PanedWindow(outer, orient="horizontal")

        cfg_holder = ttk.Frame(top_pw)
        cfg_holder.columnconfigure(0, weight=1)
        cfg_holder.rowconfigure(0, weight=1)
        self._cfg = ConfigPanel(cfg_holder, self._manager,
        ...
        top_pw.add(cfg_holder, weight=1)
        top_pw.add(viz_holder, weight=2)

        # bottom: progressbar + buttons + plot
    ...
    def _on_event(self, ev) -> None:
        ...
        if t == EventType.JOB_CREATED:
            ...
            self._progress_lbl.config(
                text=f"{label}  Ep {ep}/{n_ep}  Ret {ret:.1f}  Avg {mavg:.1f}")
    ...
    def _add_job(self) -> None:
        try:
            ...
            algo_name = self.ALGO_NAMES[tab_idx] if tab_idx < len(self.ALGO_NAMES) else "PPO"

            if ep.compare_methods:
                for name in self.ALGO_NAMES:
                    self._manager.create_job(name, self._get_algo_cfg(name), env, ep)
            else:
                self._manager.create_job(algo_name, self._get_algo_cfg(algo_name), env, ep)
    ...
    def _add_tuning_jobs(self, ep: EpisodeConfig, env: EnvConfig) -> None:
        for algo, param, val_str, _ in self._cfg._tune_items:
            algo_name = algo or self.ALGO_NAMES[self._cfg._nb.index("current")]

"""FrozenLake RL Workbench – UI Layer."""
...
class ConfigPanel(ttk.Frame):
    ...
    def _build(self):
        ...
        self._env_var = tk.StringVar(value="FrozenLake-v1")
        ttk.Entry(env_lf, textvariable=self._env_var).grid(row=1, column=1, columnspan=3, sticky="ew", padx=2, pady=1)

        ttk.Label(env_lf, text="Map", font=FONT_SMALL).grid(row=2, column=0, sticky="w", padx=(4, 2), pady=1)
        self._map_var = tk.StringVar(value="4x4")
        ttk.Combobox(env_lf, textvariable=self._map_var, values=["4x4", "8x8"], state="readonly", width=8).grid(
            row=2, column=1, sticky="w", padx=2, pady=1
        )

        self._slippery_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(env_lf, text="is_slippery", variable=self._slippery_var).grid(
            row=2, column=2, sticky="w", padx=(8, 2), pady=1
        )

        ttk.Label(env_lf, text="success_rate", font=FONT_SMALL).grid(row=3, column=0, sticky="w", padx=(4, 2), pady=1)
        self._success_var = tk.StringVar(value="1.0")
        ttk.Entry(env_lf, textvariable=self._success_var, width=8).grid(row=3, column=1, sticky="w", padx=2, pady=1)
    ...
    def get_config(self) -> AlgorithmConfig:
        ...
        cfg = AlgorithmConfig(
            ...
            env_name=self._env_var.get() or "FrozenLake-v1",
            is_slippery=self._slippery_var.get(),
    ...
    def success_rate(self) -> float:
        try:
            return float(self._success_var.get())
        except Exception:
            return 1.0
...
class WorkbenchUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("Frozen Lake RL Workbench")

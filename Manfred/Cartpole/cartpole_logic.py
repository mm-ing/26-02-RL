...
class SB3DQNAlgorithm(AlgorithmBase):
    def __init__(
        ...
        self.train_env = Monitor(gym.make(env_id, render_mode=render_mode, **kwargs))
        self.eval_env = gym.make(env_id, render_mode=render_mode, **kwargs)
        policy_kwargs = {
    ...
    def evaluate_episode(self, max_steps: int, render: bool = False) -> Dict[str, Any]:
...
class TrainLoop:
    ...
    def run_episode(self, episode_index: int, history: List[float]) -> EpisodeResult:
        ...
        evaluation = self.algorithm.evaluate_episode(self.episode_cfg.max_steps, render=True)
        with self._frame_lock:
...
class TrainingManager:
    ...
    def _clone_algorithm_config(cfg: AlgorithmConfig) -> AlgorithmConfig:
        return AlgorithmConfig(
            ...
            net=NetworkConfig(
                hidden_layers=list(cfg.net.hidden_layers),
    ...
    def _build_algorithm(self, cfg: JobConfig) -> SB3DQNAlgorithm:
        ...
        if algo_name == "dueling dqn":
            adjusted.net.hidden_layers = adjusted.net.hidden_layers + [64]
        elif algo_name in {"double + dueling dqn", "d3qn"}:
    ...
    def create_jobs_for_compare_or_tuning(
        ...
        if compare_methods:
            methods = ["DDQN", "Dueling DQN", "Double + Dueling DQN"]

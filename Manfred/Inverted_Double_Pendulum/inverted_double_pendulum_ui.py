"""
UI layer for the InvertedDoublePendulum RL Workbench.
...
class ConfigPanel(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        self._healthy_reward    = tk.DoubleVar(value=10.0)
        self._reset_noise       = tk.DoubleVar(value=0.1)
        self._render_interval   = tk.IntVar(value=10)
        self._visualize         = tk.BooleanVar(value=True)

        row(frm_env, "Healthy Reward", self._healthy_reward, 0, 0, 7)
        row(frm_env, "Noise Scale",    self._reset_noise,    0, 2, 7)
        row(frm_env, "Render ms",      self._render_interval, 1, 0, 7)
...
class TrainingStatusWindow(tk.Toplevel):
    ...
    def upsert_job(self, job_id: str, **kw) -> None:
        ...
        existing = self._tv.tag_has(job_id)
        # tag_has returns tuple of iids
...
class WorkbenchApp(tk.Tk):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("RL Workbench – InvertedDoublePendulum-v5")

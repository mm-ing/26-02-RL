"""
Logic layer for the Reacher RL Workbench.
...
def _make_env(env_c: EnvConfig):
    """Create a single Reacher-v5 env wrapped with Monitor."""
    import gymnasium as gym
    from stable_baselines3.common.monitor import Monitor
    env = gym.make(
        ENV_NAME,
        render_mode="rgb_array",
        reward_control_weight=env_c.reward_control_weight,
        reward_dist_weight=env_c.reward_dist_weight,
    )
    return Monitor(env)
...
class CheckpointManager:
    ...
    def load(save_dir: str, manager: TrainingManager) -> TrainingJob:
        ...
        ac_raw = meta.get("algo_config", {})
        # NetworkConfig is nested – reconstruct it
        if "network" in ac_raw and isinstance(ac_raw["network"], dict):
            ac_raw = dict(ac_raw)
            ac_raw["network"] = NetworkConfig(**{
                k: v for k, v in ac_raw["network"].items()
                if k in NetworkConfig.__dataclass_fields__
            })
        ac = ac_cls(**{k: v for k, v in ac_raw.items()

"""
Logic layer for the Pusher RL Workbench.
...
def _make_env(env_c: EnvConfig):
    """Create a single Pusher-v5 env wrapped with Monitor."""
    import gymnasium as gym
    ...
    env = gym.make(
        ...
        reward_near_weight=env_c.reward_near_weight,
...
def _build_model(job: TrainingJob):
    ...
    if isinstance(ac, SACConfig):
        ...
        gradient_steps = ac.gradient_steps if ac.gradient_steps != -1 else 1
        model = DDPG(
            "MlpPolicy", vec_env,
            learning_rate   = ec.alpha,
            gamma           = ec.gamma,
...
class CheckpointManager:
    ...
    def load(save_dir: str, manager: TrainingManager) -> TrainingJob:
        ...
        if os.path.exists(model_path):
            cls = {"SAC": SAC, "TD3": TD3, "DDPG": DDPG}.get(algo_name)
            if cls:

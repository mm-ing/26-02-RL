...
from cartpole_logic import AlgorithmConfig, EpisodeConfig, EventBus, JobConfig, NetworkConfig, TrainingManager, TuneConfig
...
class WorkbenchUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("RL Workbench - CartPole")
    ...
    def _build_forms(self, parent: ttk.Frame):
        ...
        self.sutton_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(env_box, text="sutton_barto_reward", variable=self.sutton_var).grid(row=1, column=0, columnspan=2, sticky="w", padx=4, pady=2)

        self.visualize_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(env_box, text="Visualize", variable=self.visualize_var).grid(row=2, column=0, sticky="w", padx=4, pady=2)

        ttk.Label(env_box, text="Frame interval (ms)").grid(row=2, column=1, sticky="w", padx=4, pady=2)
        self.frame_interval_var = tk.StringVar(value="10")
        ttk.Entry(env_box, textvariable=self.frame_interval_var, width=8).grid(row=2, column=1, sticky="e", padx=4, pady=2)
        ...
        self.alpha_var = tk.StringVar(value="0.001")
        ...
        param_box = ttk.Combobox(tune_box, textvariable=self.tune_param_var, values=["learning_rate", "gamma", "buffer_size", "batch_size"], state="readonly")
        ...
        self.tune_min_var = tk.StringVar(value="0.0005")
        ...
        self.tune_max_var = tk.StringVar(value="0.003")
        ...
        self.tune_step_var = tk.StringVar(value="0.0005")
    ...
    def _on_add_job(self):
        try:
            ...
            env_kwargs = {"sutton_barto_reward": bool(self.sutton_var.get())}

"""
UI layer for the Pusher RL Workbench.
...
class ConfigPanel(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        self._reward_ctrl_w   = tk.DoubleVar(value=0.1)
        self._reward_near_w   = tk.DoubleVar(value=0.5)
        self._reward_dist_w   = tk.DoubleVar(value=1.0)
        self._render_interval = tk.IntVar(value=10)
        self._visualize       = tk.BooleanVar(value=True)

        row(frm_env, "Ctrl Weight",  self._reward_ctrl_w,  0, 0, 7)
        row(frm_env, "Near Weight",  self._reward_near_w,  0, 2, 7)
        row(frm_env, "Dist Weight",  self._reward_dist_w,  1, 0, 7)
        row(frm_env, "Render ms",    self._render_interval, 1, 2, 7)
        ...
        self._max_steps   = tk.IntVar(value=500)
        ...
        self._tune_algo_var  = tk.StringVar(value="SAC")
        ...
        self._tune_tree = ttk.Treeview(
            tune_list_frm,
            columns=("method", "param", "values"),
            show="headings",
            height=3,
            selectmode="browse",
        )
        self._tune_tree.heading("method", text="Method")
        self._tune_tree.heading("param",  text="Parameter")
        self._tune_tree.heading("values", text="Values")
        self._tune_tree.column("method", width=60,  stretch=False)
        self._tune_tree.column("param",  width=90,  stretch=False)
        self._tune_tree.column("values", width=120, stretch=True)
        ...
        ttk.Button(btn_frm, text="Clear List",
                   command=self._tune_clear).pack(side="left", padx=2)
        ...
        self._ddpg_tab = self._build_ddpg_tab(self._nb)
        self._nb.add(self._sac_tab,  text=" SAC ")
        self._nb.add(self._td3_tab,  text=" TD3 ")
        self._nb.add(self._ddpg_tab, text=" DDPG ")
    ...
    def _build_ddpg_tab(self, nb: ttk.Notebook) -> ttk.Frame:
        tab = ttk.Frame(nb)
        tab.columnconfigure(1, weight=1)
        tab.columnconfigure(3, weight=1)

        self._ddpg_buf_size   = tk.IntVar(value=300_000)
        self._ddpg_batch      = tk.IntVar(value=256)
        self._ddpg_starts     = tk.IntVar(value=10_000)
        self._ddpg_train_f    = tk.IntVar(value=1)
        self._ddpg_grad_s     = tk.IntVar(value=-1)
        self._ddpg_tau        = tk.DoubleVar(value=0.005)
        self._ddpg_act_noise  = tk.DoubleVar(value=0.1)
        self._ddpg_hidden     = tk.StringVar(value="256,256")
        self._ddpg_activation = tk.StringVar(value="relu")

        row(tab, "Buffer Size",    self._ddpg_buf_size,  0, 0, 10)
        row(tab, "Batch Size",     self._ddpg_batch,     0, 2, 7)
        row(tab, "Learn Starts",   self._ddpg_starts,    1, 0, 10)
        row(tab, "Train Freq",     self._ddpg_train_f,   1, 2, 7)
        row(tab, "Grad Steps",     self._ddpg_grad_s,    2, 0, 10)
        row(tab, "Tau",            self._ddpg_tau,        2, 2, 7)
        row(tab, "Action Noise σ", self._ddpg_act_noise, 3, 0, 10)
        row(tab, "Hidden Layers",  self._ddpg_hidden,    4, 0, 12)
        ttk.Label(tab, text="Activation").grid(row=4, column=2, sticky="w",
                                               padx=(4, 2))
        ttk.Combobox(tab, textvariable=self._ddpg_activation,
                     values=["relu", "tanh", "elu"],
                     state="readonly", width=7).grid(row=4, column=3,
                                                     sticky="ew",
                                                     padx=(0, 4), pady=2)
    ...
    def _tune_remove(self) -> None:
        ...
        if sel:
            ...
            self._tune_items = [
                item for item in self._tune_items if item[3] != sel[0]
            ]
    ...
    def get_env_config(self) -> EnvConfig:
        return EnvConfig(
            reward_control_weight = float(self._reward_ctrl_w.get()),
            reward_near_weight    = float(self._reward_near_w.get()),
            reward_dist_weight    = float(self._reward_dist_w.get()),
    ...
    def get_ddpg_config(self, ep: EpisodeConfig) -> DDPGConfig:
        return DDPGConfig(
            buffer_size       = int(self._ddpg_buf_size.get()),
            batch_size        = int(self._ddpg_batch.get()),
            learning_starts   = int(self._ddpg_starts.get()),
            train_freq        = int(self._ddpg_train_f.get()),
            gradient_steps    = int(self._ddpg_grad_s.get()),
            tau               = float(self._ddpg_tau.get()),
            action_noise_sigma= float(self._ddpg_act_noise.get()),
            network = NetworkConfig(
                hidden_layers = self._parse_hidden(self._ddpg_hidden),
                activation    = self._ddpg_activation.get(),
            ),
        )
...
class TrainingStatusWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        for txt, cmd in [
            ...
            ttk.Button(btn_frm, text=txt, command=cmd,
    ...
    def _on_right_click(self, event: tk.Event) -> None:
        r = self._tv.identify_row(event.y)
        if r:
            self._tv.selection_set(r)
...
class WorkbenchApp(tk.Tk):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("RL Workbench – Pusher-v5")
    ...
    def _add_job(self) -> None:
        try:
            ...
            algo_name = self.ALGO_NAMES[tab_idx] if tab_idx < len(self.ALGO_NAMES) else "SAC"
    ...
    def _get_algo_cfg(self, name: str, ep: EpisodeConfig) -> Any:
        if name == "SAC":
            ...
            return self._cfg.get_ddpg_config(ep)

    def _add_tuning_jobs(self, ep: EpisodeConfig, env: EnvConfig) -> None:
        for method, param, val_str, _ in self._cfg._tune_items:
            algo_name = method or self.ALGO_NAMES[self._cfg._nb.index("current")]
    ...
    def _save_job(self) -> None:
        ...
        for job in self._manager.jobs:
            ...
            saved.append(p)
        if saved:
            messagebox.showinfo("Saved", f"Saved {len(saved)} job(s).",
                                parent=self)

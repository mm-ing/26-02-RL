"""
Logic layer for the InvertedDoublePendulum RL Workbench.
...
class SACConfig:
    algo_name:          str          = "SAC"
    buffer_size:        int          = 300_000   # capped further in _build_model
    batch_size:         int          = 256
    learning_starts:    int          = 10_000    # avoid NaN explosion early on
    train_freq:         int          = 1
...
try:
    ...
    class WorkbenchCallback(BaseCallback):
        ...
        def __init__(
            ...
            # For frame throttling: last time a frame was enqueued
        ...
        def _on_step(self) -> bool:
            ...
            if self.stop_event.is_set():
                return False  # signals SB3 to abort learn()

            # ── render frame (throttled to ~100 Hz) ──
            now = time.time()
            ...
            # ── episode end detection (SB3 VecEnv style) ──
            ...
            if dones is not None:
                for done, info in zip(dones, infos):
                    if done:
                        ...
                        # try to read loss / ent from logger
                        loss = None
                        try:
                        ...
                        epsilon = None  # not used by PPO/SAC/TD3
...
class TrainingJob:
    job_id:      str
    algo_name:   str
    algo_config: Any           # PPOConfig | SACConfig | TD3Config
    ...
    frame_queue: "queue.Queue" = field(default_factory=lambda: queue.Queue(maxsize=1), repr=False)
    error_msg:   str           = ""
...
def _run_job(job: TrainingJob, bus: EventBus) -> None:
    """Target function for the training thread of a TrainingJob."""
    try:
        ...
        # carry over previous returns
        cb._returns    = list(job.returns)
        cb._ep_count   = len(job.returns)

        job.status = JobStatus.RUNNING

        # SB3 learn – reset_num_timesteps=False allows continuation
        job.model.learn(
        ...
        # Do NOT re-raise: thread exits cleanly; error is visible via JOB_ERROR event


def _run_job_inference(job: TrainingJob, bus: EventBus, n_episodes: int = 5) -> None:
    """Run the trained model without training (inference / 'Run' mode)."""
    import gymnasium as gym
    try:
        env_c = job.env_config
        env = gym.make(
...
class CheckpointManager:
    ...
    def save(job: TrainingJob, base_dir: str) -> str:
        """Save model weights + training history. Returns the save directory."""
        ...
        meta: Dict[str, Any] = {
            ...
            "algo_config": asdict(job.algo_config) if hasattr(job.algo_config, "__dataclass_fields__") else {},
    ...
    def load(save_dir: str, manager: TrainingManager) -> TrainingJob:
        """Load a job from disk and register it with the manager."""
        from stable_baselines3 import PPO, SAC, TD3
        ...
        ec  = EpisodeConfig(**{k: v for k, v in ec_d.items() if k in EpisodeConfig.__dataclass_fields__})
        env = EnvConfig(**{k: v for k, v in env_d.items() if k in EnvConfig.__dataclass_fields__})
            ...
            if cls:
                ...
                def make_env():
                    e = gym.make(
                        ...
                        reset_noise_scale=env.reset_noise_scale,
                    )
                    return Monitor(e)
                j.model = cls.load(model_path, env=DummyVecEnv([make_env]))
        return j
...
def expand_tuning_values(raw: str) -> List[Any]:
    ...
    - Parts without commas → scalar (int if whole number, else float / str)
    - Parts with commas    → list of ints  (hidden-layer config)

    Examples:
        '64;128;256'              → [64, 128, 256]
        '1e-4;3e-4;1e-3'          → [0.0001, 0.0003, 0.001]
        '256;256,256;256,256,128' → [[256], [256, 256], [256, 256, 128]]
    """
    ...
    for p in parts:
        if not p:
            continue
        if "," in p:
            # hidden-layer config: comma-separated layer sizes
